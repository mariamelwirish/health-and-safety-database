...
FROM consult co
JOIN citizen c
  ON (c.first_name, c.last_name, c.birthdate) = (co.citizen_first_name, co.citizen_last_name, co.citizen_birthdate)
JOIN doctor d
...
FROM prescribes p
JOIN citizen ci
  ON (ci.first_name, ci.last_name, ci.birthdate) = (p.citizen_first_name, p.citizen_last_name, p.citizen_birthdate)
JOIN doctor d
...
FROM employ e
JOIN private_clinic pc ON (pc.name, pc.address) = (e.clinic_name, e.clinic_address)
...
FROM fire_logs fl
JOIN fire_department f ON (f.name, f.address) = (fl.name, fl.address)
WHERE fl.report_number=10002 AND fl.report_date='2025-10-02'
UNION ALL
-- Police
SELECT 'Police', p.name
FROM police_logs pl
JOIN police p ON (p.name, p.address) = (pl.name, pl.address)
...
SELECT i.report_number, i.report_date,
       ...
       CONCAT(c.first_name,' ',c.last_name) AS reported_by
FROM incidents i
LEFT JOIN citizen c
  ON (c.first_name, c.last_name, c.birthdate) =
     (i.reported_by_first_name, i.reported_by_last_name, i.reported_by_birthdate)
ORDER BY i.report_date, i.report_number;

/* F. Doctor referral network (who referred to whom) */
SELECT d1.medical_license_no AS referrer, CONCAT(d1.first_name,' ',d1.last_name) AS referrer_name,
       d2.medical_license_no AS referee,  CONCAT(d2.first_name,' ',d2.last_name) AS referee_name
FROM refers r
